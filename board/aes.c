/*
 * AES implementation using strategy #3 - 32-bit data-path, software, high footprint, low latency
*/
...
void print_aes_gf28_array( aes_gf28_t* s, int size ){
...
void print_aes_gf28_col( aes_gf28_col_t t ){
    printf("%X ", t & 0xFF);
    printf("%X ", (t >> 8) & 0xFF);
    printf("%X ", (t >> 16) & 0xFF);
    printf("%X ", (t >> 24) & 0xFF);
...
void print_aes_gf28_col_block( aes_gf28_col_t t_0, aes_gf28_col_t t_1, aes_gf28_col_t t_2, aes_gf28_col_t t_3  ){
    printf("%X ", t_0 & 0xFF);
    printf("%X ", t_1 & 0xFF);
    printf("%X ", t_2 & 0xFF);
    printf("%X ", t_3 & 0xFF);
    ...
    printf("%X ", (t_3 >> 8) & 0xFF);
    printf("\n");
    printf("%X ", (t_0 >> 16) & 0xFF);
    printf("%X ", (t_1 >> 16) & 0xFF);
    printf("%X ", (t_2 >> 16) & 0xFF);
    printf("%X ", (t_3 >> 16) & 0xFF);
    printf("\n");
    printf("%X ", (t_0 >> 24) & 0xFF);
    printf("%X ", (t_1 >> 24) & 0xFF);
    printf("%X ", (t_2 >> 24) & 0xFF);
    printf("%X ", (t_3 >> 24) & 0xFF);
    printf("\n");
    
}

void print_aes_gf28_row_block( aes_gf28_row_t t_0, aes_gf28_row_t t_1, aes_gf28_row_t t_2, aes_gf28_row_t t_3  ){
    printf("%X ", t_0 & 0xFF);
    printf("%X ", (t_0 >> 8) & 0xFF);
    printf("%X ", (t_0 >> 16) & 0xFF);
    printf("%X ", (t_0 >> 24) & 0xFF);
...
const uint8_t AES_RC[256] = {
  ...
  0xE8, 0xCB, 0x8D, 0x1, 0x2, 0x4, 0x8, 0x10, 
  ...
  0x1D, 0x3A, 0x74, 0xE8, 0xCB, 0x8D, 0x1, 0x2, 
  0x4, 0x8, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 
  ...
  0x8D, 0x1, 0x2, 0x4, 0x8, 0x10, 0x20, 0x40, 
  0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A, 
  0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A, 
  0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39, 
  0x72, 0xE4, 0xD3, 0xBD, 0x61, 0xC2, 0x9F, 0x25, 
  0x4A, 0x94, 0x33, 0x66, 0xCC, 0x83, 0x1D, 0x3A, 
  0x74, 0xE8, 0xCB, 0x8D, 0x1, 0x2, 0x4, 0x8, 
  ...
  0x83, 0x1D, 0x3A, 0x74, 0xE8, 0xCB, 0x8D, 0x1
};

const aes_gf28_col_t AES_ENC_TBOX_0[256] = {
...
const aes_gf28_col_t AES_ENC_TBOX_1[256] = {
...
const aes_gf28_col_t AES_ENC_TBOX_2[256] = {
...
const aes_gf28_col_t AES_ENC_TBOX_3[256] = {
...
const aes_gf28_col_t AES_ENC_TBOX_4[256] = {
...
const aes_gf28_col_t AES_DEC_TBOX_0[256] = {
...
const aes_gf28_col_t AES_DEC_TBOX_1[256] = {
...
const aes_gf28_col_t AES_DEC_TBOX_2[256] = {
...
const aes_gf28_col_t AES_DEC_TBOX_3[256] = {
...
const aes_gf28_col_t AES_DEC_TBOX_4[256] = {
...
#define AES_DEC_RND_MIX_STEP(a, b, c, d) { \
  aes_gf28_t __a09 = btime(s[a], 0x09);    \
  aes_gf28_t __b09 = btime(s[b], 0x09);    \
  aes_gf28_t __c09 = btime(s[c], 0x09);    \
  aes_gf28_t __d09 = btime(s[d], 0x09);    \
                                           \
  aes_gf28_t __a0B = btime(s[a], 0x0B);    \
  aes_gf28_t __b0B = btime(s[b], 0x0B);    \
  aes_gf28_t __c0B = btime(s[c], 0x0B);    \
  aes_gf28_t __d0B = btime(s[d], 0x0B);    \
                                           \
  aes_gf28_t __a0D = btime(s[a], 0x0D);    \
  aes_gf28_t __b0D = btime(s[b], 0x0D);    \
  aes_gf28_t __c0D = btime(s[c], 0x0D);    \
  aes_gf28_t __d0D = btime(s[d], 0x0D);    \
                                           \
  aes_gf28_t __a0E = btime(s[a], 0x0E);    \
  aes_gf28_t __b0E = btime(s[b], 0x0E);    \
  aes_gf28_t __c0E = btime(s[c], 0x0E);    \
  aes_gf28_t __d0E = btime(s[d], 0x0E);    \
                                           \
  s[a] = __a0E ^ __b0B ^ __c0D ^ __d09;    \
  s[b] = __a09 ^ __b0E ^ __c0B ^ __d0D;    \
  s[c] = __a0D ^ __b09 ^ __c0E ^ __d0B;    \
  s[d] = __a0B ^ __b0D ^ __c09 ^ __d0E;    \
}

/* Row/column packers */
void U8_TO_U32_LE_COL(aes_gf28_col_t* d, const uint8_t* s, int i) {
  *d = s[i] ^ (s[i + 4] << 8) ^ (s[i + 8] << 16) ^ (s[i + 12] << 24);
}

void U8_TO_U32_LE_ROW(aes_gf28_col_t* d, const uint8_t* s, int i) {
  *d = s[i] ^ (s[i + 1] << 8) ^ (s[i + 2] << 16) ^ (s[i + 3] << 24);
}

void U32_TO_U8_LE_COL( uint8_t* d, aes_gf28_col_t s, int i) {
  d[i] = s & 0x000000FF;
  d[i + 4] = (s >> 8) & 0x000000FF;
  d[i + 8] = (s >> 16) & 0x000000FF;
  d[i + 12] = (s >> 24) & 0x000000FF;
}

void U32_TO_U8_LE_ROW( uint8_t* d, aes_gf28_col_t s, int i) {
  d[i] = s & 0x000000FF;
  d[i + 1] = (s >> 8) & 0x000000FF;
  d[i + 2] = (s >> 16) & 0x000000FF;
...
void aes_enc_exp_step( aes_gf28_col_t* rp, const uint8_t* k ) {
 ...
 for( int i = NK; i < ( NB * ( NR + 1 ) ); i++ ) {
    ...
    if ( ( ( i % NK ) == 0 ) ) {
      t_0 = AES_RC[ (i / NK) - 1 ] ^ ( AES_ENC_TBOX_4 [ ( t_0 >> 8 ) & 0xFF ] & 0x000000FF ) ^
...
/* Takes an AES-128 plaintext m and AES-128 cipher key k, as input, and computes an AES-128 ciphertext
c as output. */
void aes_enc( uint8_t* r, const uint8_t* m, const uint8_t* k ) {
    ...
    U8_TO_U32_LE_ROW(&t_0, m, 0);
    U8_TO_U32_LE_ROW(&t_1, m, 4);
    U8_TO_U32_LE_ROW(&t_2, m, 8);
    U8_TO_U32_LE_ROW(&t_3, m, 12);

    //print_aes_gf28_row_block(t_0, t_1, t_2, t_3);

    aes_enc_exp_step(rkp, k);
    
    //print_aes_gf28_row_block(rkp[0], rkp[1], rkp[2], rkp[3]);      
    ...
    AES_ENC_RND_FINI();
    //print_aes_gf28_row_block(t_0, t_1, t_2, t_3);
    U32_TO_U8_LE_ROW(r, t_0, 0);
    U32_TO_U8_LE_ROW(r, t_1, 4);
    U32_TO_U8_LE_ROW(r, t_2, 8);
    U32_TO_U8_LE_ROW(r, t_3, 12); 
}

/* Takes an AES-128 cipher text c and AES-128 cipher key k, as input, and computes an AES-128 plaintext
m as output. */
void aes_dec( uint8_t* r, const uint8_t* c, const uint8_t* k ) {
    ...
    U8_TO_U32_LE_ROW(&t_0, c, 0);
    U8_TO_U32_LE_ROW(&t_1, c, 4);
    U8_TO_U32_LE_ROW(&t_2, c, 8);
    U8_TO_U32_LE_ROW(&t_3, c, 12);
...
int main( int argc, char* argv[] ) {
  ...
  printf("Encryption run in %lu us\n", (stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec);
  ...
  printf("Decryption run in %lu us\n", (stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec);
  
  if( !memcmp( t, m, 16 * sizeof( uint8_t ) ) ) {
    printf( "AES.Dec( k, c ) == m\n" );
  }
  else {
    printf( "AES.Dec( k, c ) != m\n" );
  }
