...
/**
  * The round function Substituion which operates on a pseudo-random permutation of the state matrix
  * \param[   out]  s (a pointer to) the current state matrix
  */
...
/**
  * The round function Substituion which operates on a fisher-yates permutation of the state matrix
  * \param[   out]  s (a pointer to) the current state matrix
  */
void aes_enc_rnd_sub_fys( aes_gf28_t* s ) {
  int T[16];
