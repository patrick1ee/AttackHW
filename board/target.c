...
/** 
  * Pre-computed round consants for key generation for up to 256 rounds
  */
...
/** 
  * Pre-computed values for 0th TBOX
  */
...
/** 
  * Pre-computed values for 1st TBOX
  */
...
/** 
  * Pre-computed values for 2nd TBOX
  */
...
/** 
  * Pre-computed values for 3rd TBOX
  */
...
/** 
  * Pre-computed values for 4th TBOX
  */
...
/**
  * Initial encryption round macro
  */
...
/**
  * Round-wise encryption round macro
  */
...
/** Produces full key expansion for required number of encryption rounds,
  * in the format of an array of 32 bit columns
  * 
  * \param[   out] rp (a pointer to) the array of round key columns
...
/** Converts 4 8 bit integers from an array into a single row format
  * 
  * \param[   out] d (a pointer to) the array of rows to write to
  * \param[in    ] s                the array of 8 bit integers to read from
  * \param[in    ] i                the index to start reading in the array from
  */
void U8_TO_U32_LE_ROW(aes_gf28_col_t* d, const uint8_t* s, int i) {
  *d = s[i] ^ (s[i + 1] << 8) ^ (s[i + 2] << 16) ^ (s[i + 3] << 24);
}

/** Converts a single row into 4 8 bit integers in a g iven array
  * 
  * \param[   out] d (a pointer to) the array of 8 bit integers to write to
  * \param[in    ] s                the row packed entity to read from
  * \param[in    ] i                the index to start writing from in the array
  */
...
void aes     ( uint8_t* c, const uint8_t* m, const uint8_t* k, const uint8_t* r ) {
  ...
  U32_TO_U8_LE_ROW(c, t_0, 0);
  U32_TO_U8_LE_ROW(c, t_1, 4);
  U32_TO_U8_LE_ROW(c, t_2, 8);
  U32_TO_U8_LE_ROW(c, t_3, 12); 
}
